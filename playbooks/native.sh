...
# this variable is used in natvie.sh
CONFIGURATION_VERSION=hawthorn.update
# this variable is used in ansible 
configuration_version=hawthorn.update
...
#cd /var/tmp/configuration/playbooks && sudo -E ansible-playbook -c local ./edx_sandbox.yml -i "localhost," $EXTRA_VARS "$@" --tags=xqueue_install
ansible_status=$?
